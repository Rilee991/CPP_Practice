...
class Graph
{
    ...
    bool dfs(int src, int k, vector<bool> &vis);
...
public:
    ...
    bool hasPathMoreThanK(int src, int k);
};
...
bool Graph::dfs(int node, int k, vector<bool> &vis) {
    vis[node] = true;
    ...
    for(pair<int,int> nbr: adj[node]) {
        ...
        if(vis[adjNode] == false) {
            if(dfs(adjNode, k - wt, vis))   return true;
        }
    }
...
bool Graph::hasPathMoreThanK(int src, int k) {
    vector<bool> vis(V, false);
    return dfs(src, k, vis);
}
...
int main()
{
	...
	g.pathMoreThanK(src, k)? cout << "Yes\n" :
							cout << "No\n";
	g.hasPathMoreThanK(src, k)? cout << "Yes\n" :
							cout << "No\n";
