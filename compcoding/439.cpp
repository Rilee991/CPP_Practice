...
pair<int,int> prefixSum(int temp[], int n) {
    unordered_map<int,int> mp;
    mp[0] = -1;
    int sr = -1, er = -1, size = 0;
    int sum = 0;
...
void solve(int mat[][MAX_COL], int row, int col) {
    int sr = -1, er = -1, sc = -1, ec = -1, size = 0;
    bool flag = false;
    ...
    for(int c1=0;c1<col;c1++) {
        ...
        for(int c2=c1;c2<col;c2++) {
            for(int r1=0;r1<row;r1++) {
                int val = mat[c2][r1] == 1 ? 1 : -1;
                temp[r1] = c2 == c1 ? val : temp[r1] + val;
            }
            pair<int,int> p = prefixSum(temp, row);
            int csr = p.first, cer = p.second, csc = c1, cec = c2;

            if(csr != -1) {
                int currSize = (cer-csr+1)*(cec-csc+1);
                if(size < currSize) {
                    sr = csr, er = cer, sc = csc, ec = cec;
                    size = currSize;
                }
            }
        }
    ...
    if(sr != -1) {
        cout<<"(Top, left): ("<<sr<<", "<<sc<<")\n";
        cout<<"(Bottom, right): ("<<er<<", "<<ec<<")\n";
    } else {
        cout<<"No such rectangle present\n";
    }
}
